...
multiwan_nft_get_dev_table()
{
	case "$1" in
		ipv4) printf '%s' "$multiwan_nft_dev_tbl_ipv4" ;;
		ipv6) printf '%s' "$multiwan_nft_dev_tbl_ipv6" ;;
		*) return 1 ;;
	esac
}

multiwan_nft_append_dev_table()
{
	local family="$1" entry="$2"

	case "$family" in
		ipv4)
			multiwan_nft_dev_tbl_ipv4="${multiwan_nft_dev_tbl_ipv4}${entry} "
			export multiwan_nft_dev_tbl_ipv4
			;;
		ipv6)
			multiwan_nft_dev_tbl_ipv6="${multiwan_nft_dev_tbl_ipv6}${entry} "
			export multiwan_nft_dev_tbl_ipv6
			;;
		*) return 1 ;;
	esac
}
...
multiwan_nft_update_dev_to_table()
{
	...
	update_table()
	{
		...
		[ "$enabled" -eq 0 ] && return
		multiwan_nft_append_dev_table "$family" "${device}=$_tid"
	}
	network_flush_cache
...
multiwan_nft_route_line_dev()
{
	...
	curr_table="$(multiwan_nft_get_dev_table "$route_family")" || return
...
multiwan_nft_delete_iface_rules()
{
	...
	for rule_id in $($IP rule list | awk -F : -v id="$id" '$1 % 1000 == id && $1 > 1000 && $1 < 4000 {print $1}'); do
...
{
	local id setname mark_val mark_hex mark_compact
	...
	mark_hex="$(printf '0x%08x' "$((mark_val))")"
	mark_compact="$(printf '0x%x' "$((mark_val))")"
	
	# Find and delete elements from sticky rule sets that contain this interface's mark
	for setname in $($NFT list sets $NFT_FAMILY $NFT_TABLE 2>/dev/null | grep -oE 'set multiwan_nft_sticky_[A-Za-z0-9_]+' | awk '{print $2}'); do
		local elements
		elements=$($NFT list set $NFT_FAMILY $NFT_TABLE "$setname" 2>/dev/null | \
			sed -n '/elements/,/}/p' | tr ',' '\n' | \
			awk -v mark="$mark_hex" -v compact="$mark_compact" '
				{
					for (i = 1; i <= NF; i++) {
						if ($i == "." && i > 1 && i < NF && ($(i + 1) == mark || $(i + 1) == compact)) {
							if ($(i - 1) ~ /^[0-9A-Fa-f:.]+$/)
								print $(i - 1) " . " $(i + 1)
						}
					}
